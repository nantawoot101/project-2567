...
export default function AddShippingAddress() {
  ...
  const fetchShippingAddress = async () => {
    try {
      const response = await axios.get(`http://localhost:8888/shipping/${id}`,
      {
        headers: {
  ...
  const hdlSubmit = async e => {
    ...
    try {
      const response = await axios.put(`http://localhost:8888/shipping/edit/${id}`, shippingAddress, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.status === 200) {
        alert('แก้ไขข้อมูลจัดส่งสำเร็จ')
      }
    } catch (error) {
      console.error('เกิดข้อผิดพลาดในการแก้ไขข้อมูลที่อยู่สำหรับจัดส่ง:', error);
